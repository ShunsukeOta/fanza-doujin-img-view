...
type FavoriteItem = FeedItem & { savedAt?: string };
...
export function FavoritesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json().catch(() => null) as FavoritesResponse | null;
      if (!response.ok || !data?.ok) throw new Error(data?.error || `お気に入りを取得できませんでした (${response.status})`);
      setItems(data.items ?? []);
      setTotal(data.total ?? 0);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "お気に入りを取得できませんでした。");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const removeFavorite = async (item: FavoriteItem) => {
    ...
    try {
      ...
      setError("お気に入りの解除に失敗しました。もう一度お試しください。");
    } finally {
      setPendingCid("");
    }
  };
    ...
    <div className="subpage-shell">
      <header className="subpage-header">
        <div>
          ...
          <h1>お気に入り</h1>
          <p className="subpage-description">保存した作品をあとからまとめて確認できます。</p>
        </div>
      ...
      <main className="subpage-content">
        ...
        {loading ? (
          <div className="subpage-state"><div className="spinner" aria-hidden="true" /><strong>お気に入りを読み込んでいます</strong></div>
        ) : error ? (
            ...
            {items.map((item) => (
              <article className="favorite-card" key={item.cid}>
                ...
                <div className="favorite-body">
                  ...
                  <div className="favorite-reactions"><span>♡ {item.likeCount.toLocaleString("ja-JP")}</span><span>保存 {item.saveCount.toLocaleString("ja-JP")}</span></div>
                  <div className="favorite-actions">
                    <button className="favorite-remove" type="button" disabled={pendingCid === item.cid} onClick={() => void removeFavorite(item)}><BookmarkIcon /> 保存解除</button>
