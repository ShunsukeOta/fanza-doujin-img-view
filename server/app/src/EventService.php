...
final class EventService
{
    ...
    {
        $pdo = $this->requirePdo();
        $this->assertRate($pdo, $anonymousUserId, 1);
        return $this->recordOne($pdo, $anonymousUserId, $sessionId, $payload);
    }

    public function recordBatch(string $anonymousUserId, string $sessionId, array $payloads): array
    {
        if ($payloads === [] || count($payloads) > 25) {
            throw new RuntimeException('events の件数が不正です。');
        }

        $pdo = $this->requirePdo();
        $this->assertRate($pdo, $anonymousUserId, count($payloads));
        $accepted = 0;
        $duplicates = 0;
        ...
        foreach ($payloads as $payload) {
            ...
            $result = $this->recordOne($pdo, $anonymousUserId, $sessionId, $payload);
            $accepted++;
            if (($result['_duplicate'] ?? false) === true) {
                $duplicates++;
            }
        }
    ...
    private function recordOne(PDO $pdo, string $uid, string $sid, array $payload): ?array
    {
        ...
        $requiresWork = $eventType !== 'session_start';
        $invalidWorkCid = $requiresWork
            && ($cid === '' || preg_match('/^[A-Za-z0-9_-]{1,128}$/', $cid) !== 1);
        $invalidOptionalCid = !$requiresWork && strlen($cid) > 128;
        if ($invalidWorkCid || $invalidOptionalCid) {
            throw new RuntimeException('cid が不正です。');
        }

        $eventId = $this->uuidOrNull($payload['eventId'] ?? null);
        $viewId = $this->uuidOrNull($payload['viewId'] ?? null);
        $version = max(1, min(99, (int)($payload['eventVersion'] ?? 3)));
        ...
        $pageIndex = isset($payload['pageIndex'])
            ? max(0, min(999, (int)$payload['pageIndex']))
            : null;
        $maxPage = isset($payload['maxPage'])
            ? max(0, min(999, (int)$payload['maxPage']))
            : null;
        $readRatio = isset($payload['readRatio'])
            ? max(0, min(1, (float)$payload['readRatio']))
            : null;
        $dwellMs = isset($payload['dwellMs'])
            ? max(0, min(3600000, (int)$payload['dwellMs']))
            : null;
        $feedId = $this->uuidOrNull($payload['feedId'] ?? null);
        $rank = isset($payload['rank'])
            ? max(1, min(200000, (int)$payload['rank']))
            : null;
        $placement = mb_substr(trim((string)($payload['placement'] ?? '')), 0, 32) ?: null;
        $landingPath = mb_substr(trim((string)($payload['landingPath'] ?? '')), 0, 512) ?: null;
        $sourceDomain = mb_substr(trim((string)($payload['sourceDomain'] ?? '')), 0, 255) ?: null;
        $campaign = mb_substr(trim((string)($payload['campaign'] ?? '')), 0, 128) ?: null;
        ...
        if (is_string($metadataJson) && strlen($metadataJson) > 4096) {
        ...
        try {
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO events '
                . '(event_id, view_id, event_version, anonymous_user_id, session_id, work_cid, event_type, '
                . 'feed_id, rank_position, page_index, max_page, read_ratio, dwell_ms, placement, landing_path, '
                . 'source_domain, campaign, metadata_json) '
                . 'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([
                $eventId,
            ...
            if ($eventId !== null && str_contains(strtolower($error->getMessage()), 'duplicate')) {
                return ['_duplicate' => true];
            }
            throw $error;
        }
    ...
    private function affinityDelta(
        ...
        $active = isset($metadata['active']) && is_bool($metadata['active'])
            ? $metadata['active']
            : null;
    ...
    private function viewDelta(int $dwell, float $ratio, ?array $metadata): float
    {
        ...
        if ($progress === 0) {
            if ($dwell < 1500) return -1.0;
            if ($dwell < 3000) return -.35;
            return min(.6, $dwell / 60000);
        }
        if ($dwell < 1000) {
            return -.25;
        }

        $dwellScore = min(1.6, $dwell / 18000);
        $readScore = min(2.2, max(0, $ratio) * 2.2);
        $progressScore = min(1.2, $progress * .3);
    ...
    private function applyGenreAffinity(PDO $pdo, string $uid, string $cid, float $delta): void
    {
        ...
        $perGenre = $delta / sqrt(count($ids));
        $upsert = $pdo->prepare(
            'INSERT INTO user_genre_scores (anonymous_user_id, genre_id, score, updated_at) '
            . 'VALUES (?, ?, ?, NOW()) '
            . 'ON DUPLICATE KEY UPDATE '
            . 'score = LEAST(20, GREATEST(-12, score * POW(0.5, TIMESTAMPDIFF(DAY, updated_at, NOW()) / 45.0) + VALUES(score))), '
            . 'updated_at = NOW()'
        );
        foreach ($ids as $id) {
            $upsert->execute([$uid, $id, $perGenre]);
        }
    }
    ...
    private function normalizeCids(array $cids): array
    {
        ...
        foreach ($cids as $cid) {
            ...
            if ($value !== '' && preg_match('/^[A-Za-z0-9_-]{1,128}$/', $value) === 1) {
                $result[$value] = true;
            }
            if (count($result) >= 100) {
                break;
    ...
    private function assertRate(PDO $pdo, string $uid, int $incoming): void
    {
        $stmt = $pdo->prepare(
            'SELECT COUNT(*) FROM events WHERE anonymous_user_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL 1 MINUTE)'
        );
        $stmt->execute([$uid]);
        if ((int)$stmt->fetchColumn() + $incoming > self::RATE_LIMIT_PER_MINUTE) {
    ...
    private function uuidOrNull(mixed $value): ?string
    {
        ...
        return preg_match('/^[a-f0-9-]{36}$/i', $normalized) === 1 ? $normalized : null;
    }
